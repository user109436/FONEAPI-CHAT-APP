...
module.exports = class Email {
  constructor(user, url) {
    ...
    this.url = url;
    this.from = `FONEAPI-CHAT-APP <${process.env.EMAIL_FROM}>`;
  ...
  async send(message, subject) {
    const mailOptions = {
      ...
      to: this.to,
      subject,
      text: htmlToText.convert(this.htmlPlain(message)),
    };
  ...
  async sendEmailVerification() {
    await this.send(
      ...
      "Email Verification - FONEAPI-CHAT-APP"
    );
  }
  async sendPasswordReset() {
    await this.send(
      "Here is your new system generated password",
      "Password Reset - FONEAPI-CHAT-APP"
    );
  }
};
